...
- `@sheet-hub/ui`: a stub React component library (🚀 powered by **shadcn/ui**)
- `@sheet-hub/eslint-config`: `eslint` configurations (includes `eslint-config-next` and `eslint-config-prettier`)
- `@sheet-hub/typescript-config`: `tsconfig.json`s used throughout the monorepo
